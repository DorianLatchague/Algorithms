...
rl.on('line', function (line) {
    if( readLines === 0 ) {
        ...
        if (followingLines === 0){
            ...
            stone = line.split(" ");
            stone.map(x => parseInt(x));
            stones.push(stone);
            if (followingLines === 1){
                console.log(`Case #${testCase}: ${solve(stones)}`);
                testCase ++;
                stones = [];
            }
            followingLines --;
...
function solve(stones, seconds = 0, order = [], currentEnergy = 0, maxEnergy = 0){
    for(let i = 0; i < stones.length; i++){
        if(!order.includes(i)){
            maxEnergy = solve(stones, seconds+stones[i][0], [...order, i], Math.max(currentEnergy+stones[i][1]-stones[i][2]*seconds, 0), maxEnergy);
        }
    }
